...
1.  **מיפוי סכימה:** סריקת כל טבלאות המערכת שקיבלנו וזיהוי העמודות בכל טבלה.
2.  **זיהוי מפתחות:** איתור מפתחות ראשיים (PK) בכל טבלה.
3.  **זיהוי קשרים:** איתור מפתחות זרים (FK) המצביעים על קשרים בין טבלאות.
4.  **בניית DSD:** יצירת תרשים זרימת נתונים המציג את ישויות המערכת והקשרים ביניהן.
5.  **הינדוס ERD:** תרגום הטבלאות והקשרים לדיאגרמת ישויות-קשרים (ERD) קונספטואלית.
...
* **מינימיזציה של פגיעה:** ביצוע שינויים על בסיס הנתונים הקיים באמצעות פקודות ALTER/UPDATE ללא בנייה מחדש של הטבלאות (לפי הנחיית המרצה).
