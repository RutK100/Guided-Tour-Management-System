...
קישור לתיקייה שצריך להוריד ולהריץ:
**[operating_instructions](/DBProject_214994642_326081148/phaseE_Shirel_Rut.zip)**
לשים לב לשים נתיב בCMD של התיקייה שבתוך התיקייה המחולצת ! (הפנימית יותר)
...
זהו מסך הבית הראשי של האפליקציה שלנו. הוא משמש כדף הניווט המרכזי שממנו אפשר להגיע לכל חלק במערכת בלחיצת כפתור.



**מה רואים במסך?**
* **נתונים כלליים (למעלה):** 4 קוביות שמציגות מידע מעודכן בזמן אמת מתוך בסיס הנתונים:
  * סך הלקוחות הרשומים.
  * סך המדריכים הקיימים.
  * כמות מופעי הטיולים.
  * סך כל החובות של המטיילים.
* **כפתורי ניווט (במרכז):** כרטיסים ברורים שמעבירים את המשתמש בלחיצה ישירות למסכי הניהול של הטבלאות (לקוחות, מדריכים, טיולים, הרשמות), למסך השאילתות ותתי-התוכניות, או לצפייה ביומן המעקב (Audit Log). 
...
### מסך ניהול וחיפוש מדריכים (Select/Read)

צילום מסך מתוך דף המדריכים (`Guides`) המציג את יכולת השליפה והסינון של המערכת.

* **טבלת המדריכים:** מציגה רשימה של המדריכים במערכת כולל שם, אימייל, טלפון ודירוג (`Rating`).
* **מנגנון החיפוש:** המשתמש מקליד אותיות בשדה החיפוש, והמערכת מסננת ומציגה באופן מיידי רק את המדריכים המתאימים.

![](/DBProject_214994642_326081148/phaseE/for_readme/6.png)

### מסך הוספת מדריך (Insert)

צילום מסך המציג את טופס ההוספה של מדריך חדש למערכת.

* **טופס הוספה (`Add New Guide`):** חלונית להזנת כל פרטי המדריך (תעודת זהות, שם, טלפון, אימייל, שכר יומי, שנות ניסיון, דירוג וכתובת).
* **פעולה:** מילוי השדות ולחיצה על שמירה מבצעים פקודת `INSERT` ישירות לטבלת המדריכים בבסיס הנתונים.
![](/DBProject_214994642_326081148/phaseE/for_readme/7.png)
### מסך אישור הוספת מדריך ושליפה

צילום מסך המציג את ההוספה המוצלחת של המדריך החדש למערכת.

* **הודעת הצלחה:** באנר ירוק למעלה המציג `Guide added successfully` ומאשר שהנתונים נשמרו ב-DB.
* **הצגת הרשומה:** חיפוש השם מציג בטבלה את המדריכה החדשה שהוספנו עם כל הפרטים המעודכנים.
![](/DBProject_214994642_326081148/phaseE/for_readme/8.png)
### מסך איתור מדריך לפני מחיקה (Delete)

צילום מסך המציג את שלב איתור הרשומה במערכת לפני ביצוע פעולת מחיקה.

* **איתור הרשומה:** המשתמש חיפש את המדריך הספציפי בטבלה כדי לוודא שזו הרשומה הנכונה.
* **מוכנות למחיקה:** בצד ימין של השורה מופיע כפתור הפח האדום (`Actions`) המאפשר למחוק את המדריך ישירות מבסיס הנתונים.
![](/DBProject_214994642_326081148/phaseE/for_readme/9.png)
### מסך מחיקת מדריך (Delete)

צילום מסך שמראה מה קורה כשרוצים למחוק מדריך מהמערכת.

* **הודעת אזהרה:** ברגע שלוחצים על כפתור הפח האדום, קופצת הודעה ששואלת את המשתמש: "האם אתה בטוח שברצונך למחוק?".
* **מחיקה מהבסיס נתונים:** אם המשתמש לוחץ על "אישור", המערכת מוחקת את המדריך הזה לצמיתות מתוך הדאטה בייס (PostgreSQL).

![](/DBProject_214994642_326081148/phaseE/for_readme/10.png)
### מסך אישור והוכחת מחיקה (Delete)

צילום מסך המוכיח שהמדריך הוסר בהצלחה ובאופן סופי מבסיס הנתונים.

* **חיפוש חוזר:** המשתמש חיפש שוב את אותו המדריך (`Augy`) לאחר ביצוע המחיקה.
* **תוצאת המחיקה:** המערכת מציגה `Found 0 records` והודעה שאין מדריכים מתאימים (`No guides match this search`), מה שמאשר שפקודת ה-`DELETE` עבדה בצורה מלאה.
![](/DBProject_214994642_326081148/phaseE/for_readme/28.png)

### מסך המצב לפני עדכון נתונים (Update)

צילום מסך שמראה את הפרטים המקוריים של המדריכים בטבלה לפני שמשנים אותם.

* **הנתונים הקיימים:** אפשר לראות בטבלה את מספרי הטלפון הישנים (לדוגמה: `0500000002`).
* **כניסה לעריכה:** לחיצה על כפתור העיפרון (השמאלי מבין שני הכפתורים בצד) תפתח את חלונית העדכון כדי שנוכל לשנות את מספר הטלפון.
![](/DBProject_214994642_326081148/phaseE/for_readme/11.png)
### מסך אישור והצגת הנתון המעודכן (Update)

צילום מסך שמראה שהשינוי נשמר בהצלחה בבסיס הנתונים.

* **הודעת הצלחה:** למעלה מופיע הבאנר הירוק `Guide updated successfully` המאשר שהעדכון בוצע.
* **הנתון החדש:** אפשר לראות בטבלה שמספר הטלפון בשורה הראשונה התעדכן בהצלחה למספר החדש.
* **בסיס הנתונים:** פעולה זו הריצה פקודת `UPDATE` ישירות על הרשומה המתאימה ב-PostgreSQL.
![](/DBProject_214994642_326081148/phaseE/for_readme/13.png)

### מסך ניהול וסינון טיולים (Select עם מסננים)

צילום מסך מתוך דף הטיולים (`Our Tours`) המציג תצוגה גרפית של הטיולים ואפשרויות חיפוש מתקדמות.

* **תצוגת כרטיסים:** הטיולים מוצגים ככרטיסים מעוצבים הכוללים את שם המסלול, רמת הקושי, אורך הקילומטרים ומשך הטיול.
* **מנגנון סינון מורכב:** בנוסף לשדה החיפוש הרגיל, המשתמש יכול לסנן את הטיולים לפי רמת קושי (`FILTER BY LEVEL`) ולפי מחיר מקסימלי לטיול (`MAXIMUM TOUR PRICE`).
![](/DBProject_214994642_326081148/phaseE/for_readme/14.png)
### מסך תוצאות סינון הטיולים (Select עם תנאים)

צילום מסך שמציג את פעולת המסננים המורכבים בזמן אמת.

* **הפעלת הסינון:** המשתמש בחר להציג רק טיולים ברמת קושי 3 (`Level 3`) ועד מחיר מקסימלי של `200` ש"ח.
* **הצגת התוצאות:** המערכת שלפה והציגה רק את הכרטיסים שרלוונטיים לדרישה זו (Route 3, Route 5, Route 8), מה שמראה ששאילתת הסינון בבסיס הנתונים עובדת בצורה מדויקת.
![](/DBProject_214994642_326081148/phaseE/for_readme/15.png)
### מסך לוח מופעי טיולים (Tour Instances by Month)

צילום מסך מתוך דף מופעי הטיולים המציג את הטיולים המתוכננים בצורה של לוח שנה שנתי.

* **חלוקה לפי חודשים:** המערכת מחלקת את מופעי הטיולים לפי חודשי השנה (ינואר, פברואר, מרץ וכו') ומציגה את כמות הטיולים בכל חודש בעיגול כתום.
* **פרטי המופע:** כל כרטיס מציג את שם המסלול, המדריך, תאריך, נקודת מפגש, מחיר, כמות מטיילים מקסימלית ואת סטטוס הטיול (COMPLETED, CANCELLED, OPEN).
* **סינון לפי שנה:** בצד ימין למעלה יש אפשרות לבחור את השנה המבוקשת כדי לשלוף מהדאטה בייס את הטיולים הרלוונטיים לאותה שנה.
![](/DBProject_214994642_326081148/phaseE/for_readme/16.png)

### מסך שאילתות ותתי-תוכניות (Queries & Programs)

צילום מסך מתוך דף השאילתות, המציג את הממשק להפעלת תהליכים מורכבים ודוחות של בסיס הנתונים.

* **תוכנית 1 - רישום לקוח לטיול (`Register Customer to Tour`):** מאפשרת לבחור לקוח וטיול, לבדוק מקומות פנויים (`Check Remaining Spots`) ולהריץ תהליך רישום מורכב בבסיס הנתונים.
* **תוכנית 2 - ניהול חובות לקוחות (`Customer Debt Management`):** מאפשרת לבחור לקוח, לבדוק את סך החובות שלו (`Check Customer Debt`) או לבצע פקודת תשלום ועדכון חובות.
* **דוחות בסיס הנתונים (`Queries`):** כפתורים מהירים בתחתית המסך להרצת שאילתות דוחות מורכבות, כגון: רווח חודשי (`Monthly Revenue`), טיולים קרובים, לקוחות עם חובות פתוחים ומדריכים פעילים.
![](/DBProject_214994642_326081148/phaseE/for_readme/18.png)
### תת-תוכנית: בדיקת חוב לקוח לפני תשלום

צילום מסך המציג את שלב בדיקת הנתונים לפני ביצוע עדכון או תשלום חוב.

* **בחירת לקוח:** נבחר לקוח ספציפי (`Customer 500`) מתוך המערכת.
* **הצגת החוב הנוכחי:** לחיצה על הכפתור `Check Customer Debt` שלפה והציגה את סך החוב הקיים בבסיס הנתונים (חוב של `₪1400`), כהכנה לביצוע פרוצדורת התשלום.
![](/DBProject_214994642_326081148/phaseE/for_readme/22.png)
### תת-תוכנית: אישור ותשלום חוב לקוח

צילום מסך המציג את המערכת לאחר הרצת פרוצדורת תשלום החוב עבור הלקוח.

* **עדכון הנתונים:** לאחר לחיצה על `Pay All Customer Debt`, המערכת איפסה את כל החובות של המשתמש.
...
### הסבר על דרך העבודה שלנו והכלים שהשתמשנו:
האפליקציה שלנו נבנתה כממשק גרפי ידידותי לניהול מערכת סיורים. בצד הלקוח השתמשנו ב־React עם TypeScript לעיצוב המסכים, הטבלאות, החיפוש והפעולות. בצד השרת השתמשנו ב־Python Flask כדי ליצור API שמקשר בין האתר לבין בסיס הנתונים. בסיס הנתונים נבנה ב־PostgreSQL ונוהל דרך pgAdmin. לצורך הרצה נוחה ואריזה השתמשנו ב־Docker Compose, שמעלה יחד את ה־Frontend, ה־Backend ואת מסד הנתונים, כך שניתן להפעיל את כל המערכת בפקודה אחת.
